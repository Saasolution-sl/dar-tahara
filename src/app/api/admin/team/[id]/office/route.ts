...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await serviceUpdate("staff_members", `id=eq.${id}`, { office_id: body.officeId });
  await serviceInsert("audit_logs", {
    actor_user_id: auth.context.user.id, action: "staff_office_assigned", resource_type: "staff_member", resource_id: id,
