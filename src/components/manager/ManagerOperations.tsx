...
function ManagerReview({ row, checklist, onAction }: { row: Assessment; checklist: Array<{ label: string; ok: boolean }>; onAction: (id: string, value: "approve" | "reject", notes: string) => Promise<void> }) {
  ...
  return <div className="border-t border-border bg-background/70 p-5 sm:p-7"><h3 className="font-serif text-xl">Required quality checks</h3><div className="mt-4 grid gap-3 md:grid-cols-2">{checklist.map(item => <div key={item.label} className={`flex gap-3 rounded-xl border p-3 text-sm ${item.ok ? "border-primary/30 bg-primary/5" : "border-amber-300 bg-amber-50"}`}><span aria-hidden>{item.ok ? "✓" : "!"}</span><span>{item.label}</span></div>)}</div>
    <div className="mt-6 grid gap-4 rounded-xl bg-secondary/50 p-4 sm:grid-cols-3"><div><p className="text-xs uppercase text-muted-foreground">Outcome</p><p className="mt-1 font-medium">{row.assessment_outcome?.replaceAll("_", " ") || "Not available"}</p></div><div><p className="text-xs uppercase text-muted-foreground">Recurring amount</p><p className="mt-1 font-medium">{row.proposed_recurring_cents === null ? "Not available" : money(row.proposed_recurring_cents)}</p></div><div><p className="text-xs uppercase text-muted-foreground">Extra services</p><p className="mt-1 font-medium">{row.additional_service_fees_cents === null ? "Not available" : money(row.additional_service_fees_cents)}</p></div></div>
