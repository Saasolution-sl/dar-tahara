...
## Website knowledge engine and WhatsApp handover

The approved Knowledge Base remains the first retrieval tier. When its best result does not clear `ASSISTANT_KNOWLEDGE_HIGH_CONFIDENCE_SCORE`, the engine searches only current, indexable content from `https://www.dartahara.com`. Website chunks use indexed full-text search and, when configured, 512-dimensional pgvector embeddings generated by one consistent model. The public chat API never returns confidence, embeddings, source URLs, article IDs, prompts, or provider telemetry.

The daily assistant job reads the production sitemap, rejects non-HTTPS and non-Dar-Tahara hosts, excludes admin, account, authentication, assessment, preview, draft, and staging paths, and follows only bounded same-host redirects. It removes scripts, styles, navigation, forms, footers, duplicate blocks, and instruction-like content before hashing and chunking the page.

Each source has immutable versions and one current searchable version. A changed page flags linked approved articles for review. When a website-backed answer is useful, the system creates or updates a `pending_review` knowledge suggestion using content-hash and optional semantic duplicate detection. A suggestion never becomes searchable until an administrator chooses **Approve and publish**.

When human help is requested, information cannot be verified, confidence is low, or self-service repeatedly fails, the website offers **Continue on WhatsApp** and **Call Dar Tahara**. Selecting either first validates the opaque browser session and saves the language, intent, inferred service/location, summary, and transcript. WhatsApp opens `https://wa.me/212623875315` with a `DT-000000` reference and concise prefilled context. `handover_requests` is channel-neutral so Dar Tahara Support or another support adapter can be changed without changing the assistant.

Additive migration:

- `supabase/migrations/20260723201716_assistant_website_knowledge_and_handover.sql`

Protected routes:

- `POST /api/chat/handover`
- `GET /api/chat/session/{id}` with `x-assistant-session-id`
- `POST /api/content/sync`
- `GET /api/content/pages`
- `GET|POST /api/admin/assistant/content`

Configuration:

```env
ASSISTANT_KNOWLEDGE_HIGH_CONFIDENCE_SCORE=5
ASSISTANT_WEBSITE_MATCH_THRESHOLD=0.18
ASSISTANT_WEBSITE_SITEMAP_URL=https://www.dartahara.com/sitemap.xml
ASSISTANT_EMBEDDING_API_URL=https://api.openai.com/v1/embeddings
ASSISTANT_EMBEDDING_API_KEY=
ASSISTANT_EMBEDDING_MODEL=text-embedding-3-small
ASSISTANT_EMBEDDING_TIMEOUT_MS=6000
```

Without an embedding key, safe indexed full-text website search remains operational. Apply the migration in staging, run a manual website sync, verify that private paths are absent, test suggestion approval/rejection, and confirm the handover row exists before the browser opens WhatsApp or the phone dialer.
...
For the production WhatsApp webhook, queue, support integration and deployment runbook, see `docs/WHATSAPP_SUPPORT.md`.
...
- account-specific booking information that requires verified identity;
