...
- **Mautic** holds marketing-relevant data only: identity, contact channel,
  ...
  referral counts and attribution, which campaigns, segments and reports
  act on.
...
- Supabase: RLS enabled **and forced** on all nine tables with **no**
  anon/authenticated policies, so access is denied by default. All writes go
  through validated server routes using the service-role key. Verification
  tokens are stored hashed, single-use, expiring.

## Mautic data model (Phase 1: done via `provision.sh`, idempotent)

- **30 custom contact fields** covering identity, billing/property summaries, service
...
- **31 tags** using controlled vocabulary (`dar-tahara`, `early-access`, `source-*`,
  `property-*`, `frequency-*`, `access-*`, verification states).
- **37 dynamic segments** covering early-access lifecycle, Moroccan residence city, cleaning
  city, language, service frequency, access method, high-intent, referred.
- **6 roles + the `dt-api` user.**

## Website ↔ Mautic sync (Phase 1: done in `src/lib/mautic/`)
...
- The page captures `src` + all `utm_*` params. First-touch values are written
  once and never overwritten; last-touch is refreshed each visit and stored in
...
3. **Supabase migration:** the migration is validated and version
